...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (storageCache.bbUrl) {
    const prs = fetch(`${storageCache.bbUrl}/rest/api/latest/inbox/pull-requests?role=REVIEWER&start=0&limit=10&avatarSize=64&withAttributes=true&state=OPEN&order=oldest`).then(response => response.json());;
    const recent = fetch(`${storageCache.bbUrl}/rest/awesome-graphs/latest/people/recent`).then(response => response.json());
